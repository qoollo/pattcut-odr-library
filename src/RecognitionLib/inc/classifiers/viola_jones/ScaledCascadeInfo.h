...
public:
	...
	ScaledCascadeInfo(const CascadeClassifier& cascade_classifier, int wnd_width, int wnd_height, double scale_factor, double step)
	{
		assert(wnd_width > 0);
		assert(wnd_height > 0);
		assert(step >= 1.0);
	...
	ScaledCascadeInfo(CascadeClassifier&& cascade_classifier, int wnd_width, int wnd_height, double scale_factor, double step)
	{
		assert(wnd_width > 0);
		assert(wnd_height > 0);
		assert(step >= 1.0);
	...
	ScaledCascadeInfo(const ScaledCascadeInfo& other)
	{
	...
	ScaledCascadeInfo(ScaledCascadeInfo&& other)
	{
	...
	ScaledCascadeInfo& operator = (const ScaledCascadeInfo& other)
	{
		if (this == &other)
			return *this; 
	...
	ScaledCascadeInfo& operator = (ScaledCascadeInfo&& other)
	{
		if (this == &other)
			return *this;
